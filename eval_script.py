...
class Detector():
    ...
    def predict(self,
        ...
        '''
        inputs, im_info = self.preprocess(str(image))
...
def predict_image(image_file, detector):
    if FLAGS.run_benchmark:
        ...
        results, pred_time = detector.predict(image_file, FLAGS.threshold)
...
def bbox2out(results, clsid2catid):
    ...
    xywh_res = []
    for dt in results:
        im_id = int(dt['image_id'])
        for dt_bbox in dt['bbox']:
            clsid, score, xmin, ymin, xmax, ymax = dt_bbox.tolist()
...
def bbox_eval(results,
    ...
    if len(xywh_results) == 0:
        print("The number of valid bbox detected is zero.\n \
...
def load_image():
    ...
    if 'annotations' not in coco.dataset:
        print('Annotation file: {} does not contains ground truth.'.format(anno_path))
...
def start_eval():
    ...
    ct = 0
    pred_times = []
    for coco_rec in records:
        result, pred_time = predict_image(coco_rec['im_file'], detector)
        coco_rec['bbox'] = result['boxes']
        pred_times.append(pred_time)
        if ct % 100 == 0:
            print('Test iter:{}'.format(ct))
        ct += 1
    with_background = False if FLAGS.num_classes == len(detector.config.labels) else True
    map_stats = bbox_eval(records, with_background=with_background)
    avg_fps = 1000. / np.mean(np.asarray(pred_times))
    alpha = 0.85
    score = alpha *  (map_stats[0] / 0.448) + (1 - alpha) * (avg_fps / 72.9)
    return_dict = {
        'score': score,
        'mAP': map_stats[0],
        'fps': avg_fps,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        required=True)
    parser.add_argument(
        "--anno_path", type=str, default='annotations/instances_val2017.json', help="coco anno_path.")
    parser.add_argument(
        "--image_dir", type=str, default='val2017', help="coco image_dir.")
    parser.add_argument(
        "--num_classes", type=int, default=1, help="num_classes.")
    parser.add_argument(
        "--run_mode",
        type=str,
        default='fluid',
    ...
    parser.add_argument(
        "--threshold", type=float, default=0.01, help="Threshold of score.")

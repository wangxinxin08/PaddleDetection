...
__all__ = ['PPYOLOv2Loss']


@register
class PPYOLOv2Loss(object):
    """
    ...
    def _get_fine_grained_loss(self,
        ...
        for i, (output, target,
            ...
            x, y, w, h, obj, cls = self._split_output(output, an_num,
                                                      num_classes)
            tx, ty, tw, th, tobj, tcls = self._split_target(target)
    ...
    def _split_target(self, target):
        ...
        tcls = fluid.layers.transpose(
            target[:, :, 5:, :, :], perm=[0, 1, 3, 4, 2])
        tcls.stop_gradient = True

        return (tx, ty, tw, th, tobj, tcls)
    ...
    def _calc_obj_loss(self, output, obj, tobj, gt_box, batch_size, anchors,
        ...
        # loss_obj_pos = fluid.layers.elementwise_mul(loss_obj_pos, pos_num, axis=0)
        loss_obj_neg = fluid.layers.reduce_sum(
            loss_obj * (1.0 - obj_mask) * iou_mask, dim=[1, 2, 3])
        # loss_obj_neg = fluid.layers.elementwise_mul(loss_obj_neg, pos_num * 3, axis=0)

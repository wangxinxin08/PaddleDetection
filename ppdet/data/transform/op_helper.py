...
def transform_bbox(bbox,
    ...
    # clip boxes
    new_bbox, mask = bbox_clip(new_bbox, w, h, area_thr)
    new_label = label[mask]
    new_score = score[mask]
    return new_bbox, new_label, new_score


def bbox_clip(bbox, w, h, area_thr=0.25, wh_thr=10, ar_thr=20):
    """
    clip bbox according to w and h
    """
    # clip boxes
    area1 = (bbox[:, 2:4] - bbox[:, 0:2]).prod(1)
    bbox[:, [0, 2]] = bbox[:, [0, 2]].clip(0, w - 1)
    bbox[:, [1, 3]] = bbox[:, [1, 3]].clip(0, h - 1)
    # compute
